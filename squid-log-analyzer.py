...
for p in args.input_paths:
    ...
    if os.path.isfile(p):
        ...
        if not pattern_filter.search(absolute_path):
            logger.debug(f'Pattern \'{filters}\' does not match \'{absolute_path}\'.')
            continue
    ...
    # if path is a directory, search through it
        ...
        for pp in os.listdir(p):
            ...
            # process files
            if os.path.isfile(subpath) and subpath not in args.input_paths:
                args.input_paths.append(subpath)
            ...
            # process directories
                ...
                if subpath not in args.input_paths:
                    args.input_paths.append(subpath)
...
if not os.path.isdir(out_dir):
    ...
    try:
        ...
        logger.error(f'Unable to create the output directory \'{out_dir}\'!')
        sys.exit(1)
